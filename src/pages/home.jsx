...
//   />
//   {/* Display formatted DD/MM/YY below input */}
//   <p className="text-xs text-gray-400 mt-1">
//     {fromDate
//       ? `${String(new Date(fromDate).getDate()).padStart(2, "0")}/${
//           String(new Date(fromDate).getMonth() + 1).padStart(2, "0")
//         }/${String(new Date(fromDate).getFullYear()).slice(-2)}`
//       : ""}
//   </p>
// </div>
...
//   // Date setup with DD/MM/YYYY format
//   const today = new Date()
//   const oneMonthAgo = new Date()
//   oneMonthAgo.setMonth(today.getMonth() - 1)
...
//         `${BASE_URL}/counts?fromDate=${formatDateForAPI(fromDate)}&toDate=${formatDateForAPI(toDate)}`
//       )
//       const data = await response.json()
//       setCounts(data)
//     } catch (error) {
...
//   const handleCardClick = (type) => {
//     navigate(
//       `/detailed-list/${type}?fromDate=${formatDateForAPI(fromDate)}&toDate=${formatDateForAPI(toDate)}`
//     )
//   }

//   // Custom input component to display DD/MM/YYYY format
//   const CustomInput = ({ value, onClick, onChange, placeholder }) => (
...
//                       onChange={(e) => handleManualDateChange(e.target.value, setFromDate)}
...
//                       onChange={(e) => handleManualDateChange(e.target.value, setToDate)}
//                       placeholder="DD/MM/YYYY"
//                     />
//                   }
//                   className="border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
...
import Header from "../components/Header"
import PieChart from "./PieChart"
import { MdDateRange, MdBookmark, MdQuestionAnswer, MdMeetingRoom, MdPhoneCallback } from "react-icons/md"
import { BASE_URL } from "../utils/constants"
import DatePicker from "react-datepicker"
import "react-datepicker/dist/react-datepicker.css"
import { format, parse, isValid } from "date-fns"
...
const Home = () => {
  ...
  const [counts, setCounts] = useState({
    ...
    total_bookings: 0,
    total_enquiry: 0,
    total_call_receive: 0,  // NEW FIELD
  })
  const [loading, setLoading] = useState(true)
  ...
  const fetchDashboardData = async () => {
    setLoading(true)
    try {
      const apiUrl = `${BASE_URL}/counts?fromDate=${formatDateForAPI(fromDate)}&toDate=${formatDateForAPI(toDate)}`
      ...
      const processedData = {
        ...
        total_enquiry: data.total_enquiry || 0,
        total_call_receive: data.total_call_receive || 0,  // This should come from backend
      }
      
      console.log("Processed counts:", processedData)
  ...
  const dashboardCards = [
    ...
    {
      title: "Call Receive",
      count: counts.total_call_receive,
      icon: <MdPhoneCallback />,
      color: "bg-blue-500",
      bgColor: "bg-blue-50",
      borderColor: "border-blue-200",
      type: "total_call_receive",
    },
  ]
  ...
  const chartData = [
    ...
    { name: "Bookings", value: counts.total_bookings, color: "#10B981" },
    { name: "Enquiry", value: counts.total_enquiry, color: "#EF4444" },
    { name: "Call Receive", value: counts.total_call_receive, color: "#3B82F6" },
  ]
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {dashboardCards.map((card, index) => (
            <div
              key={index}
              onClick={() => handleCardClick(card.type)}
              className={`${card.bgColor} ${card.borderColor} border rounded-lg p-4 cursor-pointer hover:scale-105 hover:shadow-lg transition-all duration-200`}
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600 mb-1">{card.title}</p>
